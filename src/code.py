...
notify_sensor_threshold = os.getenv('NOTIFY_SENSOR_THRESHOLD')

# 起動時に一度LINEに通知を送る。(正しくネットワークに接続していることを確認するため)
notify.pushToNtfy(f'{ntfy_endpoint}/test','Raspberry Pi Pico Wが起動しました。')
...
while True:
    ...
    print(sensor_val)
    if sensor_val > notify_sensor_threshold:
        notify.pushToNtfy(f'{ntfy_endpoint}/bell','インターホンが押されました')
